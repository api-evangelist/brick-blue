...
them rather than to a user. brick.blue's registry calls tools and reads cards for ~1,300 live
...
Every line of it is an observation with a time on it, never a judgement of the server: what
the hub called, on one occasion, and what answered. It is not a recommendation, not a trust
score and not a security review, and a domain can change hands between one look and the next.
That is why each verdict carries its own timestamp and why the receipt is worth citing
instead of the numbers — a figure repeated without its date becomes a claim about now.
